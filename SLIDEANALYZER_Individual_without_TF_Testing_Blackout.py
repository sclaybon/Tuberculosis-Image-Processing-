...
#test with an image where all of the pixels have a value of 3
image[image>0] = 3

#flatten the array check to make sure it is not too bright
#if there are less than x pixels greater than the specified value (5)
#then return no bacilli found
flattenedImage = image.flatten()
flattenedImage[flattenedImage<5] = 0
numberOfPixels = np.size(np.where(flattenedImage>0))

if numberOfPixels < 5:
	
	print("No bacilli found!")
